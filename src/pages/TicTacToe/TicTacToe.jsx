...
const TicTacToe = () => {
  ...
  const [gameStatus, statusBool] = useObjectVal(firebase.database().ref("tictactoe/"+roomId+"/gameStatus"));
  ...
  useEffect(() => {
    ...
    getIp()
  }, [data,gameStatus,winList,roomId]);
  ...
  const updateTile = (id) => {
    if(sessionData.player1.isActive && player === 1) return updateGameBoard(db,"tictactoe/"+roomId,player,id)
    if(sessionData.player2.isActive && player === 2) return updateGameBoard(db,"tictactoe/"+roomId,player,id)
  };
        ...
        <div>
         <div className="box">
          ...
          <div className="tile-container">
            {gameData && gameData.map((item,key) => (
              <div className={`tile ${checkTile(key)}`} key={key} onClick={() => updateTile(key)}>
                <div>{item}</div>
              </div>
            ))}
          </div>
